# ------------------------------------------------------------
# Activation explicite du module expérimental IterativeImputer
# ------------------------------------------------------------
from sklearn.experimental import enable_iterative_imputer  # noqa: F401

# ------------------------------------------------------------
# Imports sklearn (après l'activation !)
# ------------------------------------------------------------
...
class DataFrameProcessor(object):
    def __init__(
        ...
        df_import: 'pd.DataFrame' = None,
        limit: Optional[int] = None
    ) -> None:
        """
        - Si df_import est fourni → on l'utilise.
        - Sinon, on charge le fichier file_path (CSV ou JSONL) via load_data().
        """
        ...
        if df_import is not None:
            ...
            raise ValueError("Soit 'df_import', soit 'file_path' doit être fourni.")

        self._numeric_columns, self._ordinal_columns, self._nominal_columns = self._classify_columns()

    def __repr__(self) -> str:
        return (
            f"DataFrameProcessor(file_path='{self.file_path}', "
            f"category_threshold={self.category_threshold}, limit={self.limit})"
        )
    ...
    def _knn_imputation(
        ...
        **kwargs
        ...
        if self.df[numeric_cols].isnull().all().all():
            return
    ...
    def _cca_imputation(self, col: str = "") -> None:
        if col in self.df.columns:
            ...
            print(f"Column '{col}' not found for CCA imputation.")
    ...
    def _correlation(self, min_corr: float = 0.0) -> None:
        ...
        if drop_cols:
            ...
            print("Aucune colonne supprimée, toutes respectent le seuil.")
    ...
    def _apply_downcasting(self) -> None:
        if self.numeric_columns:
            self.df[self.numeric_columns] = self.df[self.numeric_columns].apply(
...
class AdvancedDataFrameProcessor(DataFrameProcessor):
        ...
        **kwargs
        ...
        if col not in self.df.columns:
            ...
            if similar_columns:
                ...
                raise ValueError(
                    f"Colonne '{col}' introuvable et aucune suggestion proche disponible."
                )
        ...
        try:
            ...
            print(
                "Consider reducing the DataFrame size or using a simpler "
                "imputation method."
            )
        ...
        **kwargs
        ...
        for method in methods:
            if method in methods_of_this_function:
                ...
                raise ValueError(
                    f"Method {method} not recognized in filter_irrelevant_columns."
                )
    ...
    def get_errors(self) -> dict:
        methods_of_this_function = {
            ...
            'invalid_data_types': lambda: self.df.applymap(
    ...
    def resolve_errors(self, strategies: dict = None) -> None:
        methods_of_this_function = {
            ...
            'invalid_data_types': lambda: [
                self.df.drop(columns=[col], inplace=True) for col in strategies['invalid_data_types']
            ],
    ...
    def handle_missing_values(self, strategy='mean') -> None:
        ...
        if strategy in methods_of_this_function:
            ...
            raise ValueError(
                f"Strategy '{strategy}' not recognized. Possible strategies: "
                f"{list(methods_of_this_function.keys())}"
            )
